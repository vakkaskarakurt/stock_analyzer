  ...
  template: `
    <div *ngIf="data" class="border-bottom border-white border-opacity-10 py-2" style="background: rgba(0,0,0,0.3); backdrop-filter: blur(10px);">
      <div class="container d-flex gap-5 justify-content-center justify-content-lg-start overflow-auto">
        ...
        <!-- NASDAQ Card -->
        <div class="d-flex align-items-center gap-3 border-start border-white border-opacity-10 ps-5">
          <div class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-2 py-1 rounded font-mono">NASDAQ</div>
          <span class="fw-bold text-white font-mono">{{ data.nasdaqPrice | number:'1.0-0' }}</span>
          <span class="small fw-bold font-mono" [ngClass]="data.nasdaqChange >= 0 ? 'text-success' : 'text-danger'">
            <i class="bi" [ngClass]="data.nasdaqChange >= 0 ? 'bi-arrow-up-short' : 'bi-arrow-down-short'"></i>
            %{{ data.nasdaqChange | number:'1.2-2' }}
          </span>
        </div>
      </div>
    </div>
